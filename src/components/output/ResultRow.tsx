...
import { QCRowResult } from '../../types/qc';
...
export const ResultRow: React.FC<ResultRowProps> = memo(({ row, onSelect }) => {
  return (
    ...
    >
      ...
      <td className="p-3 font-mono font-bold text-blue-700">{row.partSku}</td>
      ...
      <td className="p-3 text-slate-700">
        ...
        <span className="text-[11px] text-slate-400">{row.line}</span>
      </td>

      <td className="p-3 text-center">
        <span className={`font-bold ${row.titleMatchPct >= 70 ? 'text-emerald-700' : 'text-red-600'}`}>
          {row.titleMatchPct}%
        </span>
      </td>

      <td className="p-3 text-center">
        <span className={`font-bold ${Math.abs(row.priceVariancePct) <= 15 ? 'text-emerald-700' : 'text-red-600'}`}>
          {row.priceVariancePct > 0 ? `+${row.priceVariancePct}%` : `${row.priceVariancePct}%`}
        </span>
      </td>
      ...
      <td className="p-3 text-center">
        <span
          className={`text-[11px] font-bold px-2 py-0.5 rounded ${
            row.packQtyMatch ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'
          }`}
        >
          {row.packQtyMatch ? '1 : 1' : `${row.vendorListing.packQuantity} vs ${row.amazonListing.packQuantity}`}
        </span>
      </td>
      ...
      <td className="p-3 max-w-xs truncate text-slate-600 text-[11px]" title={row.aiVerdictReason}>
        {row.aiVerdictReason}
      </td>
